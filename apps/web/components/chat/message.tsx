...
export function Message({ message }: { message: Message }) {
  ...
  const debouncedScrollToBottom = useCallback(debounce(scrollToBottom, 100), [
    scrollToBottom,
  ]);

  useEffect(() => {
    debouncedScrollToBottom();
  }, [message.content, debouncedScrollToBottom]);
    ...
    >
      ...
      >
        ...
        <Tooltip>
          ...
          <TooltipContent>
            <p>{copiedStates[message.id] ? "Copied!" : "Copy to clipboard"}</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
